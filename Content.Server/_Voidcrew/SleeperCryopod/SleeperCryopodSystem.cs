...
using Content.Shared._Voidcrew.SleeperCryopod;
...
namespace Content.Server._Voidcrew.SleeperCryopod;
...
public sealed class SleeperCryopodSystem : EntitySystem
{
    ...
    private void OnSpawning(PlayerSpawningEvent args)
    {
        ...
        var validPods = EntityQuery<SleeperCryopodComponent>().Where(c => !IsOccupied(c)).ToList();
        foreach (var ourPod in validPods.ToList())
        {
            var owningStation = _stationSystem.GetOwningStation(ourPod.Owner);
            if (owningStation != args.Station)
                validPods.Remove(ourPod);
        }
